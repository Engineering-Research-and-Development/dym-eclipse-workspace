...
public class DymerEntryUpdateUserNotificationDefinition
	extends UserNotificationDefinition {

	public DymerEntryUpdateUserNotificationDefinition() {
		super(
			DymerPortletKeys.DYMER, 0,
			UserNotificationDefinition.NOTIFICATION_TYPE_UPDATE_ENTRY,
			"updates-a-dymer-resource");
		
		boolean defaulWebSiteStatus = GetterUtil.getBoolean(PropsUtil.get(DymerPropsKeys.DEFAULT_UPDATE_WEBSITE_NOTIFICATION_STATUS), false);

		boolean defaulEmailStatus = GetterUtil.getBoolean(PropsUtil.get(DymerPropsKeys.DEFAULT_UPDATE_EMAIL_NOTIFICATION_STATUS), false);
	...
	private static final Log _log = LogFactoryUtil.getLog(
			DymerEntryUpdateUserNotificationDefinition.class);
}